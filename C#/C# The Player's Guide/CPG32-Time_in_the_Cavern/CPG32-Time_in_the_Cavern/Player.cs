...
public class Player : Creature
{
    ...
    private void OperateFountain(string action)
    {
    ...
    private void Help()
    {
        ...
        if (Map.OnGrid(Position,"north"))
        {
            moveCommands.Add("move north");
        }
        if (Map.OnGrid(Position,"west"))
        {
            moveCommands.Add("move west");
        }
        if (Map.OnGrid(Position,"south"))
        {
            moveCommands.Add("move south");
        }
        if (Map.OnGrid(Position,"east"))
        {
            moveCommands.Add("move east");
        }

        string availableMoveCommands = String.Join(", ",moveCommands);
        Console.Write(availableMoveCommands);
        Console.WriteLine();

        if (Map.SameLocation(Map.Fountain.GetPosition(), Position))
        {
            Console.Write("Fountain: ");
            if (Map.FountainEnabled == true)
            {
                Console.Write("disable fountain");
            }
            else
            {
                Console.Write("enable fountain");
            }
            Console.WriteLine();
        }
        Console.Write("Shoot: ");
        List<string> shootCommands = new List<string>();
        shootCommands.Add("shoot north");
        shootCommands.Add("shoot west");
        shootCommands.Add("shoot south");
        shootCommands.Add("shoot east");
        string availableShootCommands = String.Join(", ",shootCommands);
        Console.Write(availableShootCommands);
        Console.WriteLine();
        Console.ForegroundColor = ConsoleColor.Gray;
    }
